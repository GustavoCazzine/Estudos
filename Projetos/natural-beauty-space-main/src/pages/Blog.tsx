...
const TopicCard = lazy(() => import('../components/blog/TopicCard'));
...
const Blog = () => {
  ...
  useEffect(() => {
    ...
    observer.current = new IntersectionObserver((entries) => {
      ...
      if (first.isIntersecting && visiblePosts < regularPosts.length) {
        setVisiblePosts((prevVisiblePosts) => {
          return Math.min(prevVisiblePosts + 2, regularPosts.length);
        });
      }
    }, { threshold: 0.1 });
  ...
  // SEO optimization - estructura de dados para artigos
  const generateSchemaOrgJSON = () => {
    const blogPostList = blogPosts.map(post => ({
      "@type": "BlogPosting",
      "headline": post.title,
      "datePublished": `2023-${post.date.slice(0, 2)}-${post.date.slice(3, 5)}`,
      "author": {
        "@type": "Person",
        "name": post.author
      },
    ...
    return {
      ...
      "@type": "Blog",
      "headline": "Blog do Natural Beauty Space",
      "description": "Dicas e tendências de beleza para sobrancelhas e cílios",
      "url": "https://naturalbeautyspace.com/blog",
  ...
  return (
    <div className={`${isMobile ? 'pt-20' : 'pt-24'} pb-16`}>
      {/* SEO Optimization */}
      <Helmet>
        <title>Blog - Natural Beauty Space | Dicas de Beleza, Cílios e Sobrancelhas</title>
        <meta name="description" content="Confira as últimas tendências e dicas sobre cílios, sobrancelhas e beleza natural. Artigos escritos por especialistas para ajudar você a cuidar da sua beleza." />
        <meta name="keywords" content="blog beleza, sobrancelhas, extensão de cílios, design de sobrancelhas, tendências de beleza" />
        <meta property="og:title" content="Blog - Natural Beauty Space" />
        <meta property="og:description" content="Artigos sobre cílios, sobrancelhas e beleza natural escritos por especialistas." />
        <meta property="og:image" content="https://naturalbeautyspace.com/images/blog-cover.jpg" />
        <meta property="og:type" content="website" />
        <meta property="og:url" content="https://naturalbeautyspace.com/blog" />
        <script type="application/ld+json">
          {JSON.stringify(generateSchemaOrgJSON())}
        </script>
        <link rel="canonical" href="https://naturalbeautyspace.com/blog" />
      </Helmet>
      ...
      <section className="py-10 md:py-16">
        <div className="container-salon">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8 px-4">
            {isLoading ? (
              // Skeleton Loader para Featured Posts
          ...
          <div className="flex flex-col lg:flex-row gap-8 px-4">
            ...
            <div className="w-full lg:w-2/3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                {isLoading ? (
                  // Skeleton Loader para Posts regulares
                  ...
                  regularPosts.slice(0, visiblePosts).map((post, index) => (
                    <div 
                      key={post.id} 
                      ref={index === visiblePosts - 1 ? lastPostElementRef : null}
                      className="bg-white rounded-xl shadow-sm overflow-hidden hover:shadow-salon transition-shadow group"
                    >
              ...
              {/* Loader ou Load More Button */}
              {!isLoading && visiblePosts < regularPosts.length && (
                <div className="mt-10 text-center">
                  <button 
                    className="btn-secondary" 
                    onClick={() => setVisiblePosts(prevVisiblePosts => Math.min(prevVisiblePosts + 2, regularPosts.length))}
                  >
              ...
              {!isLoading && visiblePosts < regularPosts.length && (
                <div className="flex justify-center mt-8">
                  <div className="w-8 h-8 border-4 border-salon-pink/30 border-t-salon-terracota rounded-full animate-spin"></div>
                </div>
              )}
            </div>
      ...
      <section className="py-10 md:py-16">
        <div className="container-salon px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {isLoading ? (
              // Skeleton loader para tópicos
              Array(8).fill(0).map((_, index) => (
                <Skeleton key={index} className="h-32 w-full rounded-xl" />
              ))
            ) : (
              <Suspense fallback={<div className="col-span-4 text-center py-8">Carregando tópicos...</div>}>
